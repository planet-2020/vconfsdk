...
class PeerConnectionClient {
  public static final String STREAM_ID = "tt-android-"+System.currentTimeMillis()+"-";
  public static final String VIDEO_TRACK_ID = STREAM_ID+"v0";
  public static final String AUDIO_TRACK_ID = STREAM_ID+"a0";
...
//  private VideoTrack remoteVideoTrack;
  private List<VideoTrack> remoteVideoTracks = new ArrayList<>();
  @Nullable
...
//  @Nullable private RecordedAudioToFileController saveRecordedAudioToFile;
  ...
  public static class PeerConnectionParameters {
    RtpTransceiver.RtpTransceiverDirection transDirection;
    boolean videoCallEnabled;
    boolean audioCallEnabled;
    int videoWidth;
    int videoHeight;
    int videoFps;
    int videoMaxBitrate;
    ...
    public PeerConnectionParameters(@NonNull PeerConnectionParameters parameters) {
      this.transDirection = parameters.transDirection;
      this.videoCallEnabled = parameters.videoCallEnabled;
      this.audioCallEnabled = parameters.audioCallEnabled;
      this.loopback = parameters.loopback;
      this.tracing = parameters.tracing;
      this.videoWidth = parameters.videoWidth;
      this.videoHeight = parameters.videoHeight;
      this.videoFps = parameters.videoFps;
      this.videoMaxBitrate = parameters.videoMaxBitrate;
      this.videoCodec = parameters.videoCodec;
      this.videoFlexfecEnabled = parameters.videoFlexfecEnabled;
      this.videoCodecHwAcceleration = parameters.videoCodecHwAcceleration;
      this.audioStartBitrate = parameters.audioStartBitrate;
      this.audioCodec = parameters.audioCodec;
      this.noAudioProcessing = parameters.noAudioProcessing;
      this.aecDump = parameters.aecDump;
      this.saveInputAudioToFile = parameters.saveInputAudioToFile;
      this.useOpenSLES = parameters.useOpenSLES;
      this.disableBuiltInAEC = parameters.disableBuiltInAEC;
      this.disableBuiltInAGC = parameters.disableBuiltInAGC;
      this.disableBuiltInNS = parameters.disableBuiltInNS;
      this.disableWebRtcAGCAndHPF = parameters.disableWebRtcAGCAndHPF;
      this.enableRtcEventLog = parameters.enableRtcEventLog;
      this.dataChannelParameters = parameters.dataChannelParameters;
    }

    public PeerConnectionParameters(RtpTransceiver.RtpTransceiverDirection transDirection,
                                    boolean videoCallEnabled,
                                    boolean audioCallEnabled,
                                    int videoWidth,
                                    int videoHeight,
    ...
    public PeerConnectionParameters(RtpTransceiver.RtpTransceiverDirection transDirection,
                                    boolean videoCallEnabled, boolean audioCallEnabled,/* boolean loopback, *//*boolean tracing,*/
      ...
      this.videoCallEnabled = videoCallEnabled;
      this.audioCallEnabled = audioCallEnabled;
      this.loopback = false/*loopback*/;
      this.tracing = false/*tracing*/;
      this.videoWidth = videoWidth;
      this.videoHeight = videoHeight;
      this.videoFps = videoFps;
      this.videoMaxBitrate = videoMaxBitrate;
    ...
    public void setTransDirection(RtpTransceiver.RtpTransceiverDirection transDirection) {
      this.transDirection = transDirection;
    }
  }
  ...
  public interface PeerConnectionEvents {
     ...
     */
    void onLocalDescription(final SessionDescription sdp, TRtcMedia rtcMedia);
  ...
  static class TRtcMedia {
    ...
    public TRtcMedia(String streamid, String mid, List<RtpParameters.Encoding> encodings) {
...
//    this.remoteSinks = remoteSinks;
  ...
  private boolean isAudioCallEnabled() {
    return peerConnectionParameters.audioCallEnabled;
  }
...
//        .setSamplesReadyCallback(saveRecordedAudioToFile)
  ...
  private void createPeerConnectionInternal() {
    ...
    List<String> mediaStreamLabels = Collections.singletonList(STREAM_ID/*"ARDAMS"*/);
    if (isVideoCallEnabled()) {
...
//            }
//        }
      events.onLocalVideoTrack(VIDEO_TRACK_ID);
...
//      if (RtpTransceiver.RtpTransceiverDirection.RECV_ONLY == peerConnectionParameters.transDirection){
//        transceiver = peerConnection.addTransceiver(MediaStreamTrack.MediaType.MEDIA_TYPE_AUDIO, transceiverInit);
//        KLog.p("RECV audio Receiver=%s, sendtrack=%s, recv track=%s",
//                transceiver.getReceiver(), transceiver.getSender().track(), transceiver.getReceiver().track());
//      }else {
//        audioSource = factory.createAudioSource(audioConstraints);
//        localAudioTrack = factory.createAudioTrack(AUDIO_TRACK_ID, audioSource);
//        localAudioTrack.setEnabled(true);
//        transceiver = peerConnection.addTransceiver(localAudioTrack, transceiverInit);
//        KLog.p("SEND audio Receiver=%s, localAudioTrack=%s, sendtrack=%s, recv track=%s", transceiver.getReceiver(), localAudioTrack,
...
//      RtpTransceiver.RtpTransceiverInit transceiverInit =
//              new RtpTransceiver.RtpTransceiverInit(peerConnectionParameters.transDirection, mediaStreamLabels
//                      /*, encodings*/ //XXX 这里不能添加 encodings否则peerConnection.addTransceiver会报错
//              );
//      RtpTransceiver videoTransceiver;
//
//      if (RtpTransceiver.RtpTransceiverDirection.RECV_ONLY == peerConnectionParameters.transDirection) {
//        videoTransceiver = peerConnection.addTransceiver(MediaStreamTrack.MediaType.MEDIA_TYPE_VIDEO, transceiverInit);
//        remoteVideoTrack = (VideoTrack) videoTransceiver.getReceiver().track();
//        remoteVideoTrack.setEnabled(true);
//        for (VideoSink remoteSink : remoteSinks) {
//          remoteVideoTrack.addSink(remoteSink); // add sink to display remote
//        }
//        KLog.p("RECV video Receiver=%s, remoteVideoTrack=%s, sendtrack=%s, recv track=%s", videoTransceiver.getReceiver(), remoteVideoTrack,
//                videoTransceiver.getSender().track(), videoTransceiver.getReceiver().track());
//      } else {
//        // if need send, need capture
//        surfaceTextureHelper = SurfaceTextureHelper.create("CaptureThread", rootEglBase.getEglBaseContext());
//        videoSource = factory.createVideoSource(false/*TODO 双流 capturer.isScreencast()*/);
...
//        localVideoTrack.addSink(localRender); // display local
//
//        videoTransceiver = peerConnection.addTransceiver(localVideoTrack, transceiverInit);
//        if (RtpTransceiver.RtpTransceiverDirection.SEND_RECV == peerConnectionParameters.transDirection) { // we also need recv remote stream
//          remoteVideoTrack = (VideoTrack) videoTransceiver.getReceiver().track();
//          remoteVideoTrack.setEnabled(true);
//          for (VideoSink remoteSink : remoteSinks) {
//            remoteVideoTrack.addSink(remoteSink); // add sink to display remote
//          }
//          KLog.p("SEND_RECV video Receiver=%s, localVideoTrack=%s, remoteVideoTrack=%s, sendtrack=%s, recv track=%s",
//                  videoTransceiver.getReceiver(), localVideoTrack, remoteVideoTrack, videoTransceiver.getSender().track(), videoTransceiver.getReceiver().track());
//        }else{
//          KLog.p("SEND video Receiver=%s, localVideoTrack=%s, sendtrack=%s, recv track=%s", videoTransceiver.getReceiver(), localVideoTrack,
//                  videoTransceiver.getSender().track(), videoTransceiver.getReceiver().track());
//        }
//        localVideoSender = videoTransceiver.getSender();
//      }
//
//    }
...
//    localVideoTrack.addSink(localRender); // TODO 通知上层让上层绑定sink
  ...
  private class PCObserver implements PeerConnection.Observer {
    ...
    public void onAddStream(
            final MediaStream stream // the stream s are simply a way to group tracks
    ) {
...
//        KLog.p("receiver=%s, track=%s, Streams.count=%s", receiver, receiver.track(), mediaStreams.length);
...
//      }
      ...
      executor.execute(() -> {
        ...
        if (isInitiator) {
          ...
          if (peerConnection.getRemoteDescription() == null) {
            ...
            updateRtpPara(true);
            rtcMedia = new TRtcMedia(STREAM_ID, getMid(localSdp.description), createEncodingList());
            events.onLocalDescription(localSdp, rtcMedia);
          } else {
          ...
          if (peerConnection.getLocalDescription() != null) {
            ...
            Log.d(TAG, "Local SDP set succesfully");
            updateRtpPara(false);
            events.onLocalDescription(localSdp, null);
            drainCandidates();
  ...
  // 更新rtp参数，主要用于simulcast
  // XXX 非常规手段，可能有问题
  private void updateRtpPara(boolean bOffer){
    KLog.p("localVideoSender=%s", localVideoSender);
    if (bOffer) {
      if (null != localVideoSender) {
        RtpParameters rtpParameters = localVideoSender.getParameters();
        KLog.p("rtpParameters.encodings.size()=%s, transactionId=%s, cname=%s",
                rtpParameters.encodings.size(), rtpParameters.transactionId, rtpParameters.getRtcp().getCname());
        for (RtpParameters.Encoding encoding : rtpParameters.encodings) {
          KLog.p("%s, %s, %s, %s, %s, %s, %s,",
                  encoding.active, encoding.maxBitrateBps, encoding.maxFramerate, encoding.minBitrateBps, encoding.rid, encoding.scaleResolutionDownBy, encoding.ssrc);
        }
        rtpParameters.encodings.clear();
        rtpParameters.encodings.addAll(createEncodingList());
      }
    }
    else{
      // TODO
//      if (null != rtcMedia && null != localVideoSender){
//        RtpParameters rtpParameters = localVideoSender.getParameters();
//        KLog.p("rtpParameters.encodings.size()=%s, transactionId=%s, cname=%s",
//                rtpParameters.encodings.size(), rtpParameters.transactionId, rtpParameters.getRtcp().getCname());
//        for (RtpParameters.Encoding encoding : rtpParameters.encodings) {
...
//      }
  ...
  void bindLocalSink(String trackId, VideoSink videoSink){
    KLog.p("trackId=%s, videoSink=%s", trackId, videoSink);
    executor.execute(() -> {
      if (VIDEO_TRACK_ID.equals(trackId)){
        // local video track
        if (null == localVideoTrack){
          KLog.p(KLog.ERROR,  "null == localVideoTrack");
          return;
        }
  ...
  void bindRemoteSink(String trackId, VideoSink videoSink){
    executor.execute(() -> {
      boolean gotIt = false;
      ...
      if (!gotIt){
        KLog.p(KLog.ERROR,  "no such track %s", trackId);
      }
  });
